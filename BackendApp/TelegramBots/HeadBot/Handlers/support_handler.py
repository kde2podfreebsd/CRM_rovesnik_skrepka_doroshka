...
async def get_new_request(message):
    ...
    try:
        ...
        logger.log(
            ...
            module_name="TelegramBots.HeadBot.Handlers.support_handler"
        )
...
async def all_passwords(call):
    try:
        ...
        logger.log(
            ...
            module_name="TelegramBots/Rovesnik/Handlers/support_handler.py"
        )
...
async def process_reservation_request(message, capacity, reservation_time, action):
    await message_context_manager.delete_msgId_from_help_menu_dict(message.chat.id)
    msg_text = ""
    if (action == "create"):
        msg_text = "Подтвердите, что вы хотите отправить запрос администратору о создании брони стола на 10+ человек или перейдите в главное меню."
    elif (action == "update"):
        msg_text = "Подтвердите, что вы хотите отправить запрос администратору о редактировании брони стола на 10+ человек или перейдите в главное меню."
    elif (action == "cancel"):
        msg_text = "Подтвердите, что вы хотите отправить запрос администратору об отмене брони стола на 10+ человек или перейдите в главное меню."
    msg = await bot.send_message(
        chat_id=message.chat.id,
        text=msg_text,
        reply_markup=InlineMarkup.reservation_request_menu(capacity, reservation_time, action),
    )
...
async def reservation_request_confirmed(message, capacity, reservation_time, action):
    await message_context_manager.delete_msgId_from_help_menu_dict(message.chat.id)
    ...
    async with async_session() as session:
        ...
        request = ""
        if (action == "create"):
            request = f"Нужно создать стол на объемом {capacity} на время {reservation_time}"
        elif (action == "update"):
            request = f"Нужно изменить стол на объем {capacity} на время {reservation_time}"
        elif (action == "cancel"):
            request = f"Нужно отменить бронь стола объемом {capacity} на время {reservation_time}"

        result = await request_dal.new_req(
    ...
    if (result != DBTransactionStatus.ROLLBACK):
        msg = await bot.send_message(
            chat_id=message.chat.id,
            text="✅ Запрос был зарегистрирован. С вами в скором времени свяжется администратор.",
            reply_markup=InlineMarkup.reservation_request_confirmed_menu(),
        )
    else:
        msg = await bot.send_message(
            chat_id=message.chat.id,
            text="❌ Запрос не был зарегистрирован. Произошла ошибка.",
            reply_markup=InlineMarkup.reservation_request_confirmed_menu(),
        )
