...
from BackendApp.TelegramBots.HeadBot.Config.bot import provider_token
...
async def invoice_menu(message):
    try:
        ...
        logger.log(
            ...
            message=f"An error occurred in invoice_handler.py in invoice_menu function: {e}",
...
async def send_referral_notification(referral: Client, referrer: Referral):
    ...
    try:
        ...
        # a client can be not authorised in the head bot, but a bot cannot send a message to them, unless they sent him a message
        # previously, resulting in not sending the referral message: 'Bad Request: chat not found'
        pass
