  ...
  const handleInputSubmit = async (data?: { strInputGuestCount?: string | undefined; strInputDate?: string | undefined; strInputTime?: string | undefined;}) => {
    ...
    const findAvailableTable = async (guestCount) => {
      ...
      if (res.data.Status === 'Failed' && res.data.Message.includes('ongoing')) {
        ...
        setAvailableTables( res.data.Message.filter( table => table.is_bowling === false && table.is_pool === false)); //filet tables where is_bowling = false and is_pool = false
        setSelectedTable(res.data.Message[0]);
        console.log('[ALL]',res.data.Message);
        console.log('[FILTERED]', res.data.Message.filter( table => table.is_bowling === false && table.is_pool === false));
      }
    };
