...
export default class TablesService {
    static async getByTimeAndCapacity(
        date: string,
        capacity: number,
    ...
    static async getAll(): Promise<AxiosResponse<{Status: string; Message: ITable[]}>> {
        return $api.get('table/get_all')
    }

    static async changeStatus(storey: number, bar_id: number) {
        return $api.patch('table/change_status_by_storey/', { storey, bar_id })
    }
}