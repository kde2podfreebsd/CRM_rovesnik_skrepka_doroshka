...
const EventList = () => {
    ...
    const isRedirect = window.location.search.includes('success')


    const [purchasedTickets, setPurchasedTickets] = useState<number[]>([]);
    useEffect(() => {
        if (isRedirect) {
            navigate('/rovesnik/my/events?barId=' + barId)
        } else {
            (async () => {
    ...
    if (!events) {
        return <div className={`w-full h-full min-h-screen ${theme === 'dark' ? 'bg-[#1C1C1C] text-neutral-100' : 'bg-[#EFEFF3]'}`}><p className='text-center'>На данный
            момент у нас не проходят никакие события</p></div>
    }

    return (
        <div className={`${events.length === 0 ? 'w-full h-full bg-neutral-200' : ''}`}>
            {!isLoading && events.length > 0 ? (
                events.length > 0 && events.filter(event => {
                    if (!event) return false; // Добавляем проверку на наличие event

                    const eventDate = event.dateandtime ? dayjs(splitDatetimeString(event.dateandtime)[0]) : null;
                    const now = dayjs();

                    return now.diff(eventDate, 'day', true) < 1;
                }).map((event) => {
                    if (!event) return null; // Добавляем проверку на наличие event
                ...
                <p className='text-center text-black'>На данный момент у нас не проходят никакие события</p>
            )}
        </div>
    );
};
